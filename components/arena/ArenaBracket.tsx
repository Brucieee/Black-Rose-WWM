...
export const ArenaBracket: React.FC<ArenaBracketProps> = ({
  ...
  // Define layout variables in component scope
  ...
  const renderMatch = (match: ArenaMatch) => {
    ...
    const renderPlayer = (player: ArenaParticipant | null, slot: 'player1' | 'player2') => {
        ...
        const score = slot === 'player1' ? ((match as any).score1 || 0) : ((match as any).score2 || 0);
        ...
        return (
            ...
            >
                {player ? (
                    ...
                    <span className="text-xs text-zinc-400 italic flex-1">Empty Slot</span>
                )}
                ...
                <div className="flex items-center">
                  ...
                  {canManage && player && onScoreUpdate && (
                      <div className="flex items-center gap-1 mr-2 bg-zinc-100 dark:bg-zinc-800 p-0.5 rounded border border-zinc-200 dark:border-zinc-700" onClick={(e) => e.stopPropagation()}>
                          <button 
                            onClick={() => onScoreUpdate(match, slot, false)} 
                            className="p-1 hover:bg-white dark:hover:bg-zinc-600 text-red-600 rounded disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
                            disabled={score <= 0}
                          ...
                          <span className="text-xs font-black w-3 text-center text-zinc-900 dark:text-zinc-100 select-none">
                            {score}
                          </span>
                  ...
                  {/* User View Score */}
                  {(!canManage || !onScoreUpdate) && player && score > 0 && (
                      <span className="text-sm font-bold w-5 text-center mr-1 text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 rounded px-1 border border-zinc-200 dark:border-zinc-700">{score}</span>
                  )}
