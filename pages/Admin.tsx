...
import { Guild, QueueEntry, GuildEvent, UserProfile, Boss, BreakingArmyConfig, ScheduleSlot, LeaderboardEntry, CooldownEntry } from '../types';
...
const Admin: React.FC = () => {
  ...
  const handleRemoveSchedule = async (index: number) => {
    ...
    // Updated to use Confirmation Modal for safety
    openDeleteModal(
  ...
  const handleResetQueue = async (e: React.MouseEvent) => {
    ...
    openDeleteModal(`Clear Queue for ${branchName}?`, "This will remove everyone from this branch's queue.", async () => {
        ...
        try {
            ...
            // Query only the queue items for the selected branch
  ...
  const handleResetCooldowns = async () => {
    ...
    openDeleteModal(`Reset Winners for ${branchName}?`, "Winners from this branch will be able to join the queue again.", async () => {
        try {
            // Filter out winners who belong to the selected branch
  ...
  const handleConfirmWinner = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We must read-modify-write or use arrayUnion with the full object
      await updateDoc(doc(db, "system", "breakingArmy"), {
  ...
  const handleDeleteLeaderboardEntry = async (e: React.MouseEvent, id: string) => {
      ...
      openDeleteModal("Delete Record?", "This leaderboard entry will be permanently removed.", async () => {
          try {
  ...
  // Filter recent winners by selected branch
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-6">
      ...
      <div className="flex space-x-1 bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg w-fit mb-8 overflow-x-auto">
        ...
        <button onClick={() => setActiveTab('winnerLogs')} className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${activeTab === 'winnerLogs' ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 shadow-sm' : 'text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-200'}`}>Winner Logs</button>
        {isAdmin && (
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
                  ...
                  <div className="overflow-x-auto">
                      <table className="w-full text-left text-sm">
                          ...
                          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                              {leaderboard.filter(e => e.status === 'verified').map(entry => (
                                  <tr key={entry.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
